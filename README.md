![Risk Management](docs/banner.png)
...
A curated, practitioner-oriented guide to **enterprise and operational risk management** — how to run gap analysis, qualitative and quantitative risk analysis, heat maps, RACI matrices, a risk register, compensating controls, continuous monitoring, and a periodic review cadence, with starting templates for each. Written for someone building or running a risk program for the first time, not just someone who already knows the vocabulary.

**Scope:** the risk-management *method* — how to identify, analyze, score, treat, document, and monitor risk — plus the standards (ISO 31000, COSO ERM, NIST SP 800-30/39, FAIR) that formalize it. Framework-specific control catalogs (NIST CSF, ISO 27001, PCI-DSS, CIS Controls) are covered by the companion [Security Frameworks](https://github.com/garynair/security-frameworks) list; sector-specific regulatory risk obligations (GLBA, NYDFS 500, HIPAA, FedRAMP) are covered by the companion sector lists (see Related Lists). This list is the general-purpose method those sector programs are built on top of.
...
- [How to Approach a Risk Program](#how-to-approach-a-risk-program)
...
Every control framework in the companion lists — NIST CSF, ISO 27001, FedRAMP, HIPAA — eventually asks the same question a different way: "what could go wrong, how bad would it be, and what are you doing about it?" Risk management is the discipline that answers that question systematically instead of reactively. Without it, a security or compliance program either treats every finding as equally urgent (wasting scarce remediation capacity) or quietly ignores gaps until an auditor or an incident forces the issue.

A mature program does five things a checklist-driven one does not: it distinguishes risks from issues (a risk is potential, an issue has already happened), it scores likelihood and impact consistently enough that a heat map means the same thing across two different business units, it names a single accountable owner for every open item, it tracks compensating controls with the same rigor as primary ones, and it reviews on a cadence rather than only when something breaks.

---

## How to Approach a Risk Program

1. **Adopt a framework before you build anything.** Pick ISO 31000 (process-focused, framework-agnostic) or COSO ERM (governance- and strategy-focused) as your organizing structure. Do not invent your own taxonomy from scratch — it will not survive contact with an auditor or a new hire.
...
3. **Build your taxonomy and governance structure.** Decide how risk is categorized (strategic, operational, financial, compliance, cyber, third-party) and who owns what under a three-lines-of-defense model before you open a single register entry.
...
5. **Analyze each risk, both ways.** Score likelihood and impact qualitatively (heat map placement) and, for anything material, quantitatively (FAIR or ALE) so a dollar figure backs the ranking, not just a color.
...
7. **Document compensating controls wherever a primary control cannot be implemented.** A compensating control needs the same rigor as the control it replaces: what it does, why it's equivalent, who approved the exception, and when it expires.
...
10. **Review and reassess continuously.** A risk register that is not touched between annual audits is a compliance artifact, not a risk-management program.

---

## Foundational Standards (ISO 31000, COSO ERM, NIST SP 800-30/39)

**Path to adoption:** all three are voluntary and non-certifiable (ISO 31000 has no certification scheme by design). Most organizations pick one as their primary methodology and use the others as reference for specific techniques (NIST SP 800-30 for a detailed qualitative/quantitative scoring method, for example).

- [ISO 31000:2018 — Risk Management Guidelines](https://www.iso.org/iso-31000-risk-management.html) - The internationally recognized, framework-agnostic risk management standard: principles, a generic framework, and a process (identify, analyze, evaluate, treat, monitor, communicate). Not certifiable and not sector-specific — designed to sit underneath any other framework.
- [ISO 31010:2019 — Risk Assessment Techniques](https://www.iso.org/standard/72140.html) - The companion standard cataloging over 40 named risk-assessment techniques (SWIFT, bow-tie analysis, FMEA, Monte Carlo, and more), each with guidance on when it applies.
- [COSO Enterprise Risk Management — Integrating with Strategy and Performance](https://www.coso.org/guidance-erm) - COSO's 2017 ERM framework, organized around five components (Governance and Culture, Strategy and Objective-Setting, Performance, Review and Revision, Information/Communication/Reporting) and closely tied to board-level strategy, distinct from COSO's separate Internal Control–Integrated Framework used for ITGC/SOX work.
...
5. Map appetite to your heat map's accept/escalate line (see Risk Heat Maps) so the two artifacts stay consistent with each other.
...
3. Map your existing organizational functions onto the three lines explicitly — most disputes about "who owns this risk" trace back to an undocumented or contested Three Lines mapping, not a genuine ambiguity in the risk itself.
...
- [COSO ERM — Governance and Culture Component](https://www.coso.org/guidance-erm) - COSO's treatment of the governance structures and organizational culture that make a Three Lines model actually function rather than exist only on paper.
...
**How to do it:**
1. Run structured workshops with business-unit owners, not just the risk/security team in a room by itself — the people running a process see risks a control catalog won't.
...
- [MITRE ATT&CK](https://attack.mitre.org/) - The most widely used threat-behavior knowledge base for identifying and modeling cyber-specific risk scenarios grounded in observed adversary techniques.
...
5. Re-run the gap analysis on a cadence (see Periodic Review Cadence), not just once at program kickoff.
...
| 2 | Repeatable | Performed consistently but not formally documented or standardized. |
| 3 | Defined | Documented, standardized, and communicated across the organization. |
| 4 | Managed | Measured and monitored with defined metrics and periodic review. |
| 5 | Optimizing | Continuously improved based on metrics and changing risk. |
...
- [ISO 31010:2019 — Risk Assessment Techniques](https://www.iso.org/standard/72140.html) - The authoritative catalog covering SWIFT, FMEA, bow-tie, Delphi, and dozens of other qualitative and semi-quantitative techniques, each with applicability guidance.
...
3. Validate loss-magnitude inputs against actual internal loss data or industry loss data (e.g., breach cost reports) wherever available, rather than relying purely on expert judgment.
...
- [FAIR Institute](https://www.fairinstitute.org/) - The nonprofit professional organization maintaining the FAIR standard, publishing the FAIR model documentation, training, and a large practitioner community.
- [Open FAIR Body of Knowledge (The Open Group)](https://www.opengroup.org/certifications/openfair) - The Open Group's formal, standardized version of FAIR (O-RA and O-RT standards), including the certification path referenced under Certifications and Training below.
- [FAIR-U](https://www.fairinstitute.org/fair-u) - The FAIR Institute's free, browser-based tool for learning and running basic FAIR risk quantification without a commercial license.
...
**What it is:** a visual grid (typically 5×5) plotting likelihood on one axis and impact on the other, with each cell color-coded (green/yellow/orange/red) to show risk severity at a glance — the single most common artifact used to communicate risk posture to people who will never read the underlying register.

**How to build one:**
1. Use the same likelihood and impact scales you defined for qualitative analysis — a heat map is a visualization of that scoring, not a separate exercise.
2. Fix your color bands to your risk appetite thresholds (see Risk Appetite above), not to an arbitrary even split of the grid — the line between "yellow" and "red" should be the line at which a risk needs escalation.
...
- [ISO 31010:2019 — Risk Assessment Techniques](https://www.iso.org/standard/72140.html) - Covers risk matrix construction and its known limitations (range compression, arbitrary color bands) as one of its cataloged techniques.
...
**How to build one:**
1. List every recurring risk-management activity down the rows (identify risk, score risk, approve treatment, implement control, test control, approve compensating control, report to board) rather than trying to build one RACI for the whole program in a single row.
...
3. Distinguish a risk (potential) from an issue (realized) explicitly — many programs run a linked but separate issues log, with a pointer back to the risk that materialised.
...
- [ISO 31000:2018](https://www.iso.org/iso-31000-risk-management.html) - Establishes the identify-analyze-evaluate-treat-monitor cycle that a risk register operationalizes as a running record.
...
- [COSO ERM — Performance Component](https://www.coso.org/guidance-erm) - COSO's treatment of selecting and prioritizing risk responses (the four T's above) as part of the ERM Performance component.
...
5. Review KRI thresholds themselves periodically — a threshold set when the organization was a tenth of its current size will generate constant false positives or false negatives if never recalibrated.

- [NIST SP 800-137](https://csrc.nist.gov/pubs/sp/800/137/final) - "Information Security Continuous Monitoring (ISCM) for Federal Information Systems and Organizations," NIST's foundational guide to building an ongoing monitoring program rather than a point-in-time assessment.
- [COSO ERM — Review and Revision Component](https://www.coso.org/guidance-erm) - COSO's component covering ongoing monitoring and the periodic reassessment of the ERM program itself, not just individual risks.
...
Not every artifact needs the same review frequency — matching cadence to volatility and stakes is itself a risk-management decision.
...
- Reassess risk appetite and tolerance statements against any strategic or organizational changes since the last full review.
...
- RACI matrix review, particularly after any organizational restructuring.
...
- Independent (third-line/internal audit) assurance review of the risk-management program itself, not just individual risks.
...
- Major organizational change: M&A, new product line, new regulatory obligation, significant system migration.
...
2. Run due diligence proportional to tier: for Critical vendors, request SOC 2 reports, security questionnaires, and evidence of their own risk-management program; for Low-tier vendors, a lighter self-attestation is often sufficient.
...
5. Feed vendor risk into the same central risk register as everything else, tagged by taxonomy, rather than maintaining it as a disconnected vendor-management artifact.
...
**What it is:** the top-level governance document that establishes the risk-management program's authority, scope, roles (tying back to your Three Lines model), methodology (which standard you follow and your scoring scales), and review cadence. Every artifact in this list should be traceable back to something the policy establishes.

**How to write one:**
1. State purpose and scope first: which business units, systems, and risk categories the policy governs.
2. Name the adopted methodology explicitly (ISO 31000, COSO ERM, or a hybrid) and reference the specific scoring scales in use, rather than restating them inline — link out to the scoring-guide template so the two artifacts can't drift apart.
...
Ready-to-use starting points, referenced throughout the sections above — copy into your own environment and adapt the scales, categories, and approval authorities to your organization:
...
- [`templates/periodic-review-checklist.md`](templates/periodic-review-checklist.md) - A checklist version of the Periodic Review Cadence section above, organized by monthly/quarterly/semi-annual/annual/event-driven interval.
...
- [ServiceNow IRM (Integrated Risk Management)](https://www.servicenow.com/products/governance-risk-and-compliance.html) - Risk-management modules built on the ServiceNow platform, commonly adopted by organizations already running ServiceNow for ITSM.
...
- [FAIR-U](https://www.fairinstitute.org/fair-u) - Free, browser-based introduction to FAIR quantitative risk analysis, no license required.
...
- [NIST Small Business Cybersecurity Corner](https://www.nist.gov/itl/smallbusinesscyber) - Free, plain-language risk-management resources aimed at organizations without a dedicated risk function.
...
- [Security Frameworks](https://github.com/garynair/security-frameworks) - A companion curated list covering NIST CSF, ISO/IEC 27001, PCI-DSS, CIS Critical Security Controls, DISA STIG, and the CRI Profile — the control catalogs a risk-register's treatment plans typically point to.
...
This list is published under [CC0 1.0 Universal](LICENSE). The linked resources retain their own licenses; COSO and ISO standards in particular are largely paywalled and not freely redistributable. Templates in the `templates/` directory are original works released under the same CC0 license — use, modify, and redistribute them freely.
